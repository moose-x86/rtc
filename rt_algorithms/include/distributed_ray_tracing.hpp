...
struct distributed_ray_tracing_shadows
{
 public:
  ...
  distributed_ray_tracing_shadows(std::shared_ptr<const rtc::scene_model> sc)
      : scene(std::move(sc))
  {
  }

  template <typename rt_serv>
  auto compute_color(const rtc::math_ray& ray, const rtc::intersection& object,
                     ...
                     const optional_color& refracted, rt_serv& rt) -> rtc::color
  {
    ...
    for (const auto& light : scene->lights)
      {
        ...
        if ((cos(n, l) > 0) || object.is_refractive(*scene))
          {
            const auto [i, f] =
                get_intesection_with_light_ray(object, ray, l, light, rt);
            ...
            if (i.is_none() || !(i < intersection(_, 1.0f)))
              {
                ...
                const auto dot_ln =
                    dot(n, nl) < 0 ? m.kts * dot(nl, -n) : dot(nl, n);
                const auto V = normalize(scene->optical_system.view_point -
                                         object.hit_point(ray));
                const auto d = inverse_square_factor(light, l);

                ilumination +=
                    f *
                    ((m.kd * dot_ln + m.ks * rtc::pow(dot(R, V), 300)) / d) *
                    light.light_color;
              }
          }
      }
    ...
    if (reflected)
      r = !m.mirror ? ((1.0f - m.ks) * r + m.ks * reflected.value())
                    : reflected.value();
 ...
 private:
  ...
  auto get_intesection_with_light_ray(const rtc::intersection& object,
                                      const rtc::math_ray& ray,
                                      const rtc::math_vector& L,
                                      const rtc::light& light, _rt& rt) const
      -> std::tuple<intersection, float>
  {
        ...
        if (intersect)
          {
            ...
            acc *= std::pow(0.5 * (intersect.attribute(*scene).kts +
                                   intersect.attribute(*scene).ktd),
    ...
    while (intersect.is_present() && intersect.is_refractive(*scene) &&
           intersect < intersection(_, 1.0f));

    assert(intersect.is_present() && intersect.is_refractive(*scene) &&
               !(intersect < intersection{_, 1.0f}) ||
           intersect.is_present() && !intersect.is_refractive(*scene) &&
               !(intersect < intersection{_, 1.0f}) ||
           intersect.is_present() && !intersect.is_refractive(*scene) &&
               (intersect < intersection{_, 1.0f}) ||
           intersect.is_none());
