...
public:
  ...
  distributed_ray_tracing_shadows(std::shared_ptr<const rtc::scene_model> sc) : scene(std::move(sc)) {}
                                      ...
                                      _rt& rt) const -> std::tuple<intersection, float>
  {
    ...
    do
    {
      if(intersect)
      {
        ...
        acc *= std::pow(0.5*(intersect.attribute(*scene).kts + intersect.attribute(*scene).ktd), length * (intersect - priv));
        priv = intersect;
      }
