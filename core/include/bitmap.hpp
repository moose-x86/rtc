...
struct color_rgb {
  ...
  color_rgb(std::uint8_t r, std::uint8_t g, std::uint8_t b) noexcept
...
struct pixel {
  ...
  pixel(std::uint16_t x, std::uint16_t y, const color_rgb& c = {}) noexcept
      : x{x}, y{y}, color{c} {}

  bool operator==(const pixel& p) const noexcept {
    return std::forward_as_tuple(x, y, color) ==
           std::forward_as_tuple(p.x, p.y, p.color);
  }
};
...
class bitmap : std::vector<rtc::pixel> {
  ...
  struct header {
    ...
    std::uint8_t
        biClrImportant;  // The number of important colors in the palette
 ...
 public:
  ...
  [[nodiscard]] const color_rgb& at(const std::uint16_t,
                                    const std::uint16_t) const;
  const color_rgb& operator()(const std::uint16_t, const std::uint16_t) const;
  auto assign(const std::uint16_t, const std::uint16_t,
              const color_rgb&) noexcept -> bool;

  auto revert() noexcept -> bitmap&;
  bitmap operator!() const;
  auto swap(bitmap& bmp) noexcept(
      noexcept(std::declval<base>().swap(std::declval<base&>()))) -> bitmap&;
  ...
  [[nodiscard]] auto trim(const std::uint16_t, const std::uint16_t,
                          const std::uint16_t, const std::uint16_t) const
      -> bitmap;

  bitmap& draw(const std::function<rtc::color_rgb(std::uint16_t,
                                                  std::uint16_t)>&) noexcept;

 private:
  void reset() noexcept;
  [[nodiscard]] auto in_range(const std::uint16_t p_x,
                              const std::uint16_t p_y) const noexcept -> bool;
  header header_data;
};

inline auto operator==(const bitmap& a, const bitmap& b) noexcept -> bool {
  return (a.pixel_amount() == b.pixel_amount()) &&
