...
public:
  ...
  log(const std::string& file, const std::uint32_t line) : log()
  {
    ...
    if(rtc_unlikely(!syslog))
      syslog = std::make_unique<std::ofstream>("rsyslog");
    ...
    *syslog 
            ...
            // << ":"
            // << duration_cast<milliseconds>(now - zero).count() % milliseconds{1s}.count()
            << "] DEBUG_LOG ["
