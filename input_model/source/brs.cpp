...
void brs::check_if_all_files_exist(const std::initializer_list<std::string>& files)
{
  for (const auto& s : files)
    {
      std::ifstream f{s};
      if (!f.good()) throw std::runtime_error("file " + s + " can not be openned");
    }
}
              ...
              if (nn.first == "p")
                {
                  ...
                  if (name == "kd") sm.kd = std::stof(nn.second.data());

                  if (name == "ks") sm.ks = std::stof(nn.second.data());

                  if (name == "kts") sm.kts = std::stof(nn.second.data());

                  if (name == "ktd") sm.ktd = std::stof(nn.second.data());

                  if (name == "ka") sm.ka = std::stof(nn.second.data());

                  if (name == "kf") sm.kf = std::stof(nn.second.data());

                  if (name == "gs") sm.gs = std::stof(nn.second.data());

                  if (name == "gm") sm.gm = std::stof(nn.second.data());

                  if (name == "ft") sm.ft = std::stof(nn.second.data());

                  if (name == "eta") sm.eta = std::stof(nn.second.data());

                  if (name == "selfLuminance") sm.selfLuminance = std::stof(nn.second.data());

                  if (name == "heatLam") sm.heatLam = std::stof(nn.second.data());

                  if (name == "oneSheet") sm.oneSheet = nn.second.data() == "true" ? true : false;

                  if (name == "mirror") sm.mirror = nn.second.data() == "true" ? true : false;

                  if (name == "reflection") sm.reflection = nn.second.data() == "true" ? true : false;

                  if (name == "indirect") sm.indirect = nn.second.data() == "true" ? true : false;

                  if (name == "shadowcast") sm.shadowcast = nn.second.data() == "true" ? true : false;

                  if (name == "shadowfall") sm.shadowfall = nn.second.data() == "true" ? true : false;

                  sm.shading = surface_material::shading_type::guard;
                }
              else if (nn.first == "color")
                {
                  sm.material_color = rtc::color{nn.second.get<rtc_float>("r"), nn.second.get<rtc_float>("g"),
      ...
      if (n.first == "camera")
        {
          ...
          c.view_point = {n.second.get<rtc_float>("position.<xmlattr>.x"),
                          n.second.get<rtc_float>("position.<xmlattr>.y"),
                          n.second.get<rtc_float>("position.<xmlattr>.z")};
