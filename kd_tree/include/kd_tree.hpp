...
class kd_tree {
 ...
 public:
  ...
  [[nodiscard]] auto cbegin(const rtc::math_ray&) const noexcept
      -> const_iterator;
  [[nodiscard]] auto cend(const rtc::math_ray&) const noexcept
      -> const_iterator;
 ...
 private:
  ...
  rtc_hot void build_tree(
      std::unique_ptr<tree_node>& node, rtc::bounding_box b,
      std::vector<std::uint32_t> tr,
      const std::vector<rtc::bounding_box>& primitive_bboxes,
      edge_buffer_array_t& edges, const std::uint32_t depth,
      std::uint32_t bad_refines = 0);

  rtc_hot auto compute_node_split_paramters(
      edge_buffer_array_t& edges, const std::vector<std::uint32_t>& tr,
