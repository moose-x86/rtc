...
kd_tree::kd_tree(const rtc::scene_model& ss) : bbox{ss.points} {
  ...
  auto make_bbox = [](auto& p, auto& tr) noexcept {
    return rtc::bounding_box{p[tr.vertex_a()], p[tr.vertex_b()],
                             p[tr.vertex_c()]};
  };
  ...
  edge_buffer_array_t edge_buffer{edge_buffer_t(2 * t.size()),
                                  edge_buffer_t(2 * t.size()),
                                  edge_buffer_t(2 * t.size())};

  primitive_bboxes.reserve(t.size());

  std::for_each(t.begin(), t.end(),
                [&](auto& t) { primitive_bboxes.push_back(make_bbox(p, t)); });
  std::generate(v.begin(), v.end(), [i = 0]() mutable { return i++; });

  build_tree(root, node_bbox, std::move(v), primitive_bboxes, edge_buffer,
             max_depth);
}

auto kd_tree::cbegin(const rtc::math_ray& ray) const noexcept
    -> kd_tree::const_iterator {
...
auto kd_tree::cend(const rtc::math_ray&) const noexcept
    -> kd_tree::const_iterator {
  return const_iterator{};
}
...
void kd_tree::build_tree(std::unique_ptr<tree_node>& node,
                         ...
                         const std::uint32_t depth, std::uint32_t bad_refines) {
  ...
  if (tr.size() <= 1 || !depth) return create_leaf_node();

  const auto [axis, offset, cost, old_cost] = compute_node_split_paramters(
      edge_buffer, tr, node_bbox, primitive_bboxes);

  bad_refines += (cost > old_cost) ? 1 : 0;

  auto no_use_of_spliting_node = [old_cost = old_cost, cost = cost, axis = axis,
                                  &bad_refines, &tr] {
    return ((cost > 4 * old_cost) && (tr.size() < 16)) || (axis == -1) ||
           (bad_refines == 3);
  };

  if (no_use_of_spliting_node()) return create_leaf_node();

  auto [left_set, right_set] =
      split_triangles(std::move(tr), edge_buffer, axis, offset);
  ...
  build_tree(node->left, left_bbox, std::move(left_set), primitive_bboxes,
             edge_buffer, depth - 1, bad_refines);
  build_tree(node->right, right_bbox, std::move(right_set), primitive_bboxes,
             edge_buffer, depth - 1, bad_refines);
}

auto kd_tree::compute_node_split_paramters(
    edge_buffer_array_t& edge_buffer, const std::vector<std::uint32_t>& tr,
  ...
  do {
    ...
    for (const auto i : boost::irange<std::size_t>(0, tr.size())) {
      ...
      edge_buffer[int(axis)][2 * i] = {box.min_boundary().axis(axis), tr[i],
                                       bounding_edge_point::start};
      edge_buffer[int(axis)][2 * i + 1] = {box.max_boundary().axis(axis), tr[i],
                                           bounding_edge_point::end};
    }

    std::sort(&edge_buffer[int(axis)][0],
              &edge_buffer[int(axis)][0] + 2 * tr.size());
    ...
    for (const auto i : boost::irange<std::size_t>(0, 2 * tr.size())) {
      ...
      if (edge_t > node_bbox.min_boundary().axis(axis) &&
          edge_t < node_bbox.max_boundary().axis(axis)) {
        const rtc_float belowSA{
            2 * (node_diagonal.axis(axis2) * node_diagonal.axis(axis3) +
                 (edge_t - node_bbox.min_boundary().axis(axis)) *
                     (node_diagonal.axis(axis2) + node_diagonal.axis(axis3)))};

        const rtc_float aboveSA{
            2 * (node_diagonal.axis(axis2) * node_diagonal.axis(axis3) +
        ...
        const rtc_float cost{traversal_cost +
                             isect_cost * (1 - eb) *
                                 (pBelow * below + pAbove * above)};
...
auto kd_tree::split_triangles(std::vector<std::uint32_t>&& tr_init,
    ...
    -> vector_tuple<std::uint32_t, std::uint32_t> {
